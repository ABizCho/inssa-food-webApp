...
**1-3 카카오 소셜 로그인 구현:** 카카오 소셜 로그인 기능을 구현하면서도 적지않은 어려움을 겪었다. 카카오에 Token을 발급받기 위해 카카오 developers api에 요청을 보낼 때 처음에 적절한 곳에 요청을 보내지 못했었는데 처음엔 요청의 문제인지도 인지하지 못했다, 이를 발견하기 위해 로그인 관련 로직 대부분에 콘솔을 찍어보며 문제 위치를 찾아낼 수 있었다. 처음부터 예외처리와 오류로그를 분명히 명시하도록 개발했더라면 비교적 문제를 금방 찾아낼 수 있었을 것이라고 생각하며 에러처리 로직을 사전에 작성해 둘 중요성을 분명히 느낄 수 있었다.
<br><br>
